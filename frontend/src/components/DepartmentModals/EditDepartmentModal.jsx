// components/DepartmentModals/EditDepartmentModal.jsx
import React, { useState, useEffect } from 'react';
import axios from '../../api';

const EditDepartmentModal = ({ isOpen, onClose, onSuccess, department, companies }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`/api/departments/${department.deptID}`, payload);
      ...
      if (error.response && error.response.data && error.response.data.errors) {
        ...
        setErrors({ general: 'Failed to update department. Please try again.' });
      }
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[#25424D] rounded-lg w-full max-w-lg p-6 text-white">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-gray-400 text-sm mb-1">Department ID</label>
            <input
              type="text"
              value={department.deptID}
              readOnly
              className="w-full bg-[#1F3A45] rounded p-2 opacity-60 cursor-not-allowed"
            />
          </div>
